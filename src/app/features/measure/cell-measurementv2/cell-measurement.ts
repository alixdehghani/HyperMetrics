...
@Component({
  ...
  selector: 'app-cell-measurement-v2',
...
export class CellMeasurementComponentV2 implements OnInit, OnDestroy {
  ...
  validateCounterNameV2(counter: Counter, measureObj: MeasureObj): string[] {
    const errors: string[] = [];
    if (!counter.name || !counter.name.trim()) {
      errors.push("Counter name is required.");
    }
    const existingNames = measureObj.counterList.filter(c => c.id !== counter.id).map(c => c.name);
    if (existingNames.some(existingName => existingName === counter.name)) {
      errors.push(`${counter.name} is already used by another counter. Counter names must be unique.`);
    }
    return errors;
  ...
  private _normalizeCountersAndKpis(measureObjData: measurementData) {
    ...
    measureObjData.measureObjList.forEach((measureObj, measureIndex) => {
      ...
      measureObj.counterList.forEach((counter, i) => {
        // If a counter with the same name was already assigned earlier, reuse its numeric id
        const existingCounter = measureObjData.measureObjList
          .filter(mo => mo.measureObjId !== measureObj.measureObjId) // exclude current measureObj
          .flatMap(m => m.counterList)
          .find(c => c.name === counter.name && c._numericId);

        if (existingCounter) {
          console.log(existingCounter);

          // counterIdSeq = Number(existingCounter._numericId);
          counter.id = existingCounter.id;
        } else {
  ...
  getAllCountersErrors(measureObj: any): string[] {
    ...
    for (let counter of measureObj.counterList) {
      const counterErrors = this.validateCounterNameV2(counter, measureObj);
  ...
  confirmAddNewCounter(measureObj: MeasureObj) {
    ...
    // Validate name uniqueness
    const nameErrors = this.validateCounterNameV2(newCounter, measureObj);
    if (nameErrors.length > 0) {
  ...
  confirmCounterNameChange(counterFormControl: AbstractControl, counter: Counter): boolean | void {
    ...
    const measureObj = this.measurementData.measureObjList.find(mo =>
      mo.counterList.some(counterItem => counterItem.id === counter.id)
    );
    if (!measureObj) {
      alert("Error: Could not find parent measurement object for this counter.");
      this.cancelCounterEdit(counterFormControl, counter);
      return;
    }
    if (this.validateCounterNameV2(counterFormControl.value, measureObj).length > 0) {
      alert("❌ Invalid counter name:\n" + this.validateCounterNameV2(counterFormControl.value, measureObj).join('\n'));
      this.cancelCounterEdit(counterFormControl, counter);
      return;
    }
